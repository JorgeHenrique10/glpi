...
class Ticket extends CommonITILObject {
   ...
   **/
   ...
   function pre_updateInDB() {      
      ...
      if(strstr($this->input['name'], 'SETOR CONTRATOS')) {

        $data_atual = date("Y-m-d H:i:s");
        $hora_atual = date("H:i:s", strtotime($data_atual));
        
        $ticket_status = new Tickets_Status();
        $ticket_status_time = new Ticket_Status_Time();

        $time_ticket =  $ticket_status_time->getStatus($this->input['status'], $this->fields['id']);

        //$date_increment = date("Y-m-d H:i:s", strtotime("+" . $time_ticket . " days", strtotime($data_atual)));
        $date_increment = date('Y-m-d H:i:s', strtotime($data_atual .' +'. $time_ticket .' Weekday'));
        $date_increment = str_replace('00:00:00', $hora_atual, $date_increment);
        
        if($calendar->isHoliday($date_increment)){          
          $date_increment = date('Y-m-d H:i:s', strtotime($date_increment .' +1 Weekday'));
          $date_increment = str_replace('00:00:00', $hora_atual, $date_increment);     
        }

        $this->updates[]                = 'time_to_own';
        $this->input['_time_to_own']    = $date_increment;
        $this->input['time_to_own']     = $date_increment;
        $this->fields['time_to_own']    = $date_increment;                       
        
        
        $status_anterior = $ticket_status->ant_status($this->fields['id'], $data_atual);

        $ticket_status->alter_status($this->fields['id'], $this->fields['status'], $data_atual);        
   ...
   **/
   ...
   /**
    * get the Ticket status list from contract
    *
    * @param $withmetaforsearch boolean (false by default)
    *
    * @return array
   **/
   ...
   static function getAllStatusArray($withmetaforsearch = false) {
      ...
      $tab = [     self::INCOMING => _x('status', 'New'),
                   self::ASSIGNED => _x('status', 'Processing (assigned)'),
                   self::PLANNED  => _x('status', 'Processing (planned)'),
                   self::QUALIFICATION  => _x('status', 'Qualificação'),
                   self::ATTACH  => _x('status', '1-Atribuido'),
                   self::VALIDATION_TR  => _x('status', '2-Validação TR'),
                   self::PUBLICATION  => _x('status', '3-Publicação'),
                   self::SEEM_ENABLING  => _x('status', '4-Parecer de Habilitação'),
                   self::VALIDATION_TECHNIQUE  => _x('status', '5-Validação Tecnica'),
                   self::RESULT  => _x('status', '6-Resultados'),
                   self::HOMOLOGATION  => _x('status', '7-Homologação'),
                   ...
                   self::CANCELADO  => _x('status', 'Cancelado'),
                   self::WAITING  => __('Pending'),
                   self::SOLVED   => _x('status', 'Solved'),
                   self::CLOSED   => _x('status', 'Closed')];        
   ...
   **/
   ...
   function showStatsDates() {
      ...
      $dates_check = Tickets_Status::show_states($this->fields['id']);
      ...
      $dates = $dates_ini + $dates_check + $dates_end;
