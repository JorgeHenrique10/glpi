   ...
   static function constructData(array &$data, $onlycount = false) {
      ...
      if ($result) {
         ...
         while (($i < $data['data']['totalcount']) && ($i <= $data['data']['end'])) {
            ...
            if(isset($data['data']['rows'][$i]['raw']['ITEM_Ticket_1']))
            {
              if(strstr (trim($data['data']['rows'][$i]['raw']['ITEM_Ticket_1']) , 'SETOR CONTRATOS'))
              {
                $status_cod = $data['data']['rows'][$i]['Ticket_158'][0]['status'];
                $id_ticket = $data['data']['rows'][$i]['raw']['ITEM_Ticket_1_id'];
                $data_ini_ticket = Tickets_Status::getDataIniTicketAtual($id_ticket, $status_cod);
                
                $time = Ticket_Status_Time::getStatus($status_cod, $id_ticket);
                $data_inicial = new DateTime($data_ini_ticket);
                $data_final = new DateTime(date('Y-m-d H:i:s'));
                $interval = $data_inicial->diff($data_final);
                $interval ="&nbsp &nbsp &nbsp &nbsp &nbsp". $interval->d . '/' . $time;

                //$data['data']['rows'][$i]['raw']['ITEM_Ticket_158'] = $interval->d . '/' . $time;
                $data['data']['rows'][$i]['Ticket_158']['displayname'] = $time ? $interval : '';
              }
            }
