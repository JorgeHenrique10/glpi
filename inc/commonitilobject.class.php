...
abstract class CommonITILObject extends CommonDBTM {
   ...
   const VALIDATION_TR        = 13; // validação tr
   const PUBLICATION          = 14; // publicacao 
   const SEEM_ENABLING        = 15; // parecer de habilitação 
   const VALIDATION_TECHNIQUE = 16; // validação tecnica
   const RESULT               = 17; // resultados 
   const HOMOLOGATION         = 18; // homologação 
   const JURIDICAL            = 19; // juridico 
   const VALIDATION_INTERNAL  = 20; // validação interna 
   ...
   **/
   ...
   function showStatsTimes() {
      ...
      $lista_times = Tickets_Status::show_times($this->fields['id']);

      foreach ($lista_times as $item) {
        echo "<tr class='tab_bg_2'><td>". $item['name'] ."</td><td>";
        if ($item['timestamp'] > 0) {
            $time = '';
            if($item['timestamp']->d > 0)
              $time.= $item['timestamp']->d . ' dias ';
            if($item['timestamp']->h > 0)
              $time.= $item['timestamp']->h . ' horas ';
            if($item['timestamp']->i > 0)
              $time.= $item['timestamp']->i . ' minutos ';
            if($item['timestamp']->s > 0)
              $time.= $item['timestamp']->s . ' segundos ';
