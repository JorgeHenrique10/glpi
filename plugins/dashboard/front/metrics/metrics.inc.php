...
"SELECT avg( `glpi_ticketsatisfactions`.satisfaction ) AS media
	...
	$query_stat_m = "
	SELECT AVG(Total) as media_diaria from (
		SELECT 
			count(glpi_tickets.id) as Total, date
		FROM glpi_tickets
	...
	$result_stat_m = $DB->query($query_stat_m);
	
	$media_diaria = $DB->result($result_stat_m,0,'media_diaria');
	
	
	$assigned_m = $assig_m + $plan_m;
	$total_m = $new_m + $assig_m + $plan_m + $pend_m + $solved_m;
	
	//Count by create today

	$query_new_today ="
	select count(glpi_tickets.id) as 'Criado_Hoje' 
	from glpi_tickets
		 where glpi_tickets.is_deleted = '0'
		 and year(date_creation) = year(now())
		 and month(date_creation) = month(now())
	...
	$result_stat_td = $DB->query($query_new_today);
	$c_hj = $DB->result($result_stat_td,0,'Criado_Hoje');
